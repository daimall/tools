...
type CommFlow struct {
	...
	DataCache map[string]interface{} `gorm:"-" json:"-"` // 缓存上下文需要的数据
}
...
// 流程步骤（下一步/上一步）
func (x *CommFlow) GoNext(tx *gorm.DB, flow FlowService, tblName, serviceName string, remainingIds []int64) (curStepHandlers []FlowHandler, err error) {
	var (
		step                FlowStep
		remainCheckHandlers []CheckHandler
	)
	if step, err = flow.GetCurStep(); err != nil {
	...
	if pass*100/total >= step.PassRate() {
		if next, err = flow.GetNextStep(); err != nil {
	...
	if nopass*100/total > 100-step.PassRate() {
		// 退回流程
		if next, err = flow.GetPreStep(); err != nil {
	...
	if next == nil {
		...
		if preStep {
			...
			if curStep, err = flow.GetCurStep(); err != nil {
...
func (f *CommFlow) SetData(k string, v interface{}) {
	if f.DataCache == nil {
		f.DataCache = make(map[string]interface{})
	}
	f.DataCache[k] = v
}

func (f *CommFlow) GetData(k string) interface{} {
	return f.DataCache[k]
}
