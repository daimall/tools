...
type CheckHandler struct {
	...
	ServiceId   int64  `gorm:"column:service_id;unique_index:onerecord"  json:"serviceId"`    // FlowId
...
func (h *CheckHandler) Do(uname string, c common.BaseController) (ret interface{}, oplog string, err error) {
	...
	if curStepHandlers, err = h.Flow.GoNext(tx, checkHandler.PreStepHandlerIds); err != nil {
		return
	}
...
// 从数据库中加载数据，返回一个新实例
func (h *CheckHandler) LoadInst(flow FlowService, uname string, id int64) (ret FlowHandler, err error) {
...
// 加载handler
func (h *CheckHandler) LoadStepHandlers(tx *gorm.DB, flowId int64, stepKey string) (handlers []FlowHandler, err error) {
	l := []CheckHandler{}
