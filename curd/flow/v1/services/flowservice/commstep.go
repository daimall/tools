...
type CommStep struct {
	...
	Configs            []flowcomm.ConfigPage // 处理该步骤需要对参数对象
	LoadConfig         func(flow FlowService) []flowcomm.ConfigPage
...
// 流程步骤等操作信息
func (s *CommStep) LoadHandlers(tx *gorm.DB, flowId int64) (handlers []FlowHandler, err error) {
	if s.Handler == nil {
