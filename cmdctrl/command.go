...
func (cc *CommandCtrl) UpdateArgs(name string, args ...string) error {
	...
	logs.Info("cmd args: %v", pkeeper.cmdInfo.Args)
	if !pkeeper.keeping {
		return nil
	}
...
func (p *processKeeper) start() error {
	...
	go func() {
		for {
			...
			if p.cmdInfo.ArgsFunc != nil {
				...
				if er != nil {
					logs.Info("ArgsFunc error: %v", er)
					goto CMD_DONE
				}
			}
			logs.Info("[%s] Args: %v", p.name, cmdArgs)
			if p.cmdInfo.Shell {
			...
			logs.Info("[%s] args: %v, env: %v", p.name, cmdArgs, p.cmdInfo.Environ)
			if err := p.cmd.Start(); err != nil {
				goto CMD_DONE
			}
			logs.Info("[%s] program pid: %d", p.name, p.cmd.Process.Pid)
			...
			case cmdErr := <-cmdC:
				if cmdErr != nil {
					logs.Info("[%s] cmd wait err: %v", p.name, cmdErr)
				}
		...
		CMD_IDLE:
			logs.Info("[%s] idle for %v", p.name, p.cmdInfo.NextLaunchWait)
			p.running = false
			select {
			case <-p.stopC:
				goto CMD_DONE
	...
	CMD_DONE:
		logs.Info("[%s] program finished", p.name)
